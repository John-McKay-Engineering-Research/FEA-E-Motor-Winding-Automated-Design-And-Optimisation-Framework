...
def mirror_Coil(conductor_Array, shape_Array, scale_Array, rotation_Array):
    ...
    for x in range(len(conductor_Array)):
        ...
        if conductor_Centroid.x < half_Way_Point:
            ...
            # now check if this shape is a triangle, if yes move the conductor again as flipping it may displace it
            # from the original position.
            if shape_Array[x] == "Triangle":
                conductor = move_Conductor(conductor, position_x, position_y)  # just in case its drifted.
        ...
        if conductor_Centroid.x == half_Way_Point:
            # need to change this as non uniform conductors arent being flipped on the x-axis like triangles
            # conductor = conductor
            position_x = conductor.centroid.x
            position_y = conductor_Centroid.y
...
def Experiment_2(shape_List, scale_List, x_Position_Array, y_Position_Array, rotation_List):
    # create file paths
    ...
    if Is_Random_Test == True:
        ...
        for x in range(Number_of_conductors):
            ...
            conductor_ = scale_Conductor(conductor_, scale_List[x])
            scale_Array.append(scale_List[x])
            ...
            if conductor_Shape == 3:  # "Triangle"
                conductor_ = move_Conductor(conductor_, x_Position_Array[x], (y_Position_Array[x]))
                        # y_Position_Array[x] - (scale_List[x] * 0.0535)))  # length ratio is the scale.
            else:
                conductor_ = move_Conductor(conductor_, x_Position_Array[x], y_Position_Array[x])
    ...
    mirror_Conductor_Array = mirror_Coil(conductor_Array, shape_Array, scale_Array, rotation_Array)
    ...
    # *** EDITS ***
    # 1 :: edited the mirror coil function where the conductors is directly in the centre, now gets flipped in case it's
    # a non-uniform conductor like a triangle.
    #
    # 2 :: edited mirror coil function, added a check for triangular conductor, if conductor is triangular the conductor
    # will be moved again to x and y positions calculated. just in case they drifted.
