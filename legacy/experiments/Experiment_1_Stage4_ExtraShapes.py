...
sys.argv.append('C:/Users/John McKay/PycharmProjects/coil_Configurator/optislang_Interconnect_Orderly_Stage2.py')
...
coil_Center_2 = shapely.geometry.Point(51.65235222, 124.6998093)  # these values have to be converted into negatives when exporting.
# ---------------------------------------------------------------------------------------------------------------------
...
def rotate_Conductor(conductor, Angle):
    ...
    conductor = shapely.affinity.rotate(conductor, Angle, conductor.centroid)
    return conductor
...
def move_Triangular_Conductor(conductor, x_Position, y_Position, conductor_Scale, conductor_Rotation):
    conductor_Centre = conductor.centroid  # the current centre of the shape.
    move_To = shapely.geometry.Point(x_Position, y_Position)
    # code below moves the shape around the slot.
    # triangle conductors need a set of rules to rotate them depending on the rotation
    conductor = shapely.affinity.translate(conductor, xoff=(move_To.x), yoff=(move_To.y - (conductor_Scale * 0.0535)))
    print("x co-ordinates : ", move_To.x)
    print("Y co-ordinates : ", move_To.y)
    # print("centroid of this conductor is " + str(conductor.centroid))
    return conductor
...
def create_Coil(conductor_Array, coil_Point, coil_Angle, coil_Array):
    ...
    for y in range(len(coil_Array)):
        ...
        difference = shapely.geometry.Point(coil_Point.x - slot_Centroid.x, coil_Point.y - slot_Centroid.y)  # calculate the difference between the original slot in shapely space to conductor slot in maxwell space.
...
def mirror_Coil(conductor_Array, shape_List, rotation_Array):
    ...
    for x in range(len(conductor_Array)):
        ...
        print("\n")
        print("Mirror_Coil : conductor shape = ", shape_List[x], x, "conductor rotation = ", rotation_Array[x], "conductor centroid = ", conductor_Centroid)
        ...
        if conductor_Centroid.x < half_Way_Point:
            ...
            print("Mirror_Coil : Position x : ", position_x, " Position y : ", position_y)
            conductor = move_Conductor(conductor, position_x, position_y)
            conductor = mirror_Slot(conductor, -1, 1)  # we want to mirror only the x axis not the y. this should handle a rotated conductor.
            ...
            conductor = move_Conductor(conductor, position_x, position_y)
            print("conductor centroid : ", conductor.centroid)
            print("conductor was mirrored")
            print("\n")
...
def Experiment_1(scale_Array, shape_List, x_Position_Array, y_Position_Array, rotation_Array):
    # create file paths
    ...
    if Is_Orderly_Test_2 == True:
        ...
        for x1 in range(len(grid_Array)):
            ...
            if shape_List[x1] == 3:  # is "Triangle" ***
                ...
                # conductor_ = move_Conductor(conductor_, x_Position_Array[x1], (y_Position_Array[x1] - (scale_Array[x1] * 0.15)))  # original 0.0535
                conductor_ = move_Conductor(conductor_, grid_Array[x1].centroid.x,
                                            (grid_Array[x1].centroid.y - (scale_Array[x1] * 0.0535)))  # original 0.0535
                print("Triangle conductor centroid after moving is : ", conductor_.centroid.x, conductor_.centroid.y)
            else:
                # shape_Array.append(conductor_Shape) # keep tabs on what each polygon is
                conductor_ = move_Conductor(conductor_, x_Position_Array[x1], y_Position_Array[x1])

            # print("grid array position ", x1, " centroid is ", grid_Array[x1].centroid.x, " , ",  grid_Array[x1].centroid.y)
            print("scale array position x line 619 in Experiment_1.py : , ", x1)
            ...
            if shape_List[x1] == 3:  # is "Triangle" ***
                ...
                # conductor_ = rotate_Special_Conductor(conductor_, rotation_Array[rotation_Counter], x_Position_Array[x1], y_Position_Array[x1])
                # conductor_ = rotate_Special_Conductor(conductor_, rotation_Array[x1], x_Position_Array[x1], (y_Position_Array[x1] - (scale_Array[x1] * 0.15)))

                conductor_ = rotate_Special_Conductor(conductor_, rotation_Array[x1], grid_Array[x1].centroid.x, (grid_Array[x1].centroid.y - (scale_Array[x1] * 0)))
                print("Triangle conductor centroid after rotating is : ", conductor_.centroid.x, conductor_.centroid.y)
            else:
                conductor_ = rotate_Conductor(conductor_, rotation_Array[rotation_Counter])
    ...
    mirror_Conductor_Array = mirror_Coil(conductor_Array, shape_List, rotation_Array)
    ...
    excel_Tool_Extra_Shapes.write_Coil_To_Excel_Experiment_Version(shape_Array, conductor_Array, scale_Array, rotation_Array, coil_Weight, fill_Factor, machine_Efficiency, machine_Losses, new_Directory, slot_Configuration)
