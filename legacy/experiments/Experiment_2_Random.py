...
def Experiment_2():
    ...
    scale_Array = [] # we dont pass scale parameters in the random distribution. C and N does that for us.
    # start the experiment.

    if Is_Random_Test_2 == True:

        # conductor_Array = [] # conductor array

        zeta_C = 1.3 # not less than 1.1
        zeta_N = 5 # not greater than 3
        ...
        for x in range(Number_of_conductors):
            ...
            while conductor_Is_Colliding(conductor_, conductor_Array) == True:
                ...
                if conductor_Shape == "Triangle":
                    conductor_ = move_Conductor(conductor_, random_Position.x, (random_Position.y - (length_Ratio * 0.0535))) # length ratio is the scale.
                else:
