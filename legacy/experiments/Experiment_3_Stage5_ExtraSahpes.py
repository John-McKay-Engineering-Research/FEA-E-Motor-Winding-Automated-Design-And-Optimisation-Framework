...
import sys
import additional_Shapes
import vbs_Script_Creator_ExperimentVer_2_Extra_Shapes
import excel_Tool_Extra_Shapes
...
Is_hairpin = False
Is_layered = True  # a layered slot has a dominant shape per layer with each layer varying in height.
number_Of_Layers = 10
...
# coil_Center_1 = shapely.geometry.Point(17.68236713, 134.3109071)
...
# sys.argv.append('C:/Users/John McKay/PycharmProjects/coil_Configurator/optislang_Interconnect_Layers_Stage1_OptiVer.py')
# sys.argv[0] = r'C:/Users/John McKay/PycharmProjects/coil_Configurator/optislang_Interconnect_Layers_Stage1.py'
# sys.argv.append(r'C:/Users/John McKay/PycharmProjects/coil_Configurator/optislang_Interconnect_Layers_Stage1.py')
sys.argv.append('C:/Users/John McKay/PycharmProjects/coil_Configurator/optislang_Interconnect_Layers_Stage1.py')
print("path to sys.arg is : ", sys.argv)

# print("value in sys.argv[2] : ", sys.argv[2])
...
def rotate_Special_Conductor(conductor, Angle, x_Point, y_Point):
    # this exists to rotate conductors with off centre centroids to the shape area, aka non squares such as triangles etc
    rotation_Point = shapely.geometry.Point(x_Point, y_Point)
    conductor = shapely.affinity.rotate(conductor, Angle, rotation_Point)
    return conductor
...
def mirror_Coil(conductor_Array):
    ...
    for x in range(len(conductor_Array)):
        ...
        if conductor_Centroid.x < half_Way_Point:
            ...
            # FIX **, adding in  another move conductor to try and fix conductor location being broken when the conductor is flipped on the x-axis.
            conductor = move_Conductor(conductor, position_x, position_y)
...
def get_Grid():
    ...
    ratio = (Slot_Size_x) / (
        Slot_Size_Y)  # *** for safety reasons we take 0.1 of a mm off so that the conductors do not touch the slot in the simulation. ***
...
def get_Coil_Mass(coil_Mass_Array):
    ...
    conductor_Length = (
                210 + 25 + 25)  # 210mm for the stator iron length, 2.5cm * 2 for the end winding length. from cm to mm

    for x in range(len(coil_Mass_Array)):
        coil_Final_Mass += ((coil_Mass_Array[
                                 x] * conductor_Length) / 1000000000) * 8940  # convert mm3 into m3 then multiply by density kg/m3 to get value in kg.
...
def Experiment_3(layer_Count, layer_Array, shape_List, scale_List, x_Position, y_Position, rotation_List):
    # create file paths

    # need number of layers, number of conductors per layer and the starting size Y of each layer.
    layer_Count = layer_Count
    conductors_Per_Layer = Number_of_conductors / layer_Count
    layer_Size_Array = []  # an array of layer sizes

    # layers should add up to 100%, each layer will grow and shrink if other layers increase or decrease in height.
    for z in range(layer_Count):
        layer_ = layer_Array[z] / 100  # layers are real number, need to turn to floats
        layer_Size_Array.append(layer_)
    ...
    if Is_layered == True:
        ...
        for x in range(len(layer_Size_Array)):
            print("layer number = ", x)

            # firstly get the grid
            grid_Height = layer_Size_Array[x] * Slot_Size_Y

            grid_Tuple = get_Grid_For_Layer(grid_Height, conductors_Per_Layer)
            number_Of_Rows = grid_Tuple[0]
            number_Of_Cols = grid_Tuple[1]
            grid_Size = grid_Tuple[
                2]  # Max size a conductor can be minus a small area to prevent conductor intersection.

            print("grid values", grid_Tuple)  # rows, columns, size.
            # conductor_Array = []
            ...
            for y in range(number_Of_Rows):
                ...
                while x_Counter < number_Of_Cols:
                    ...
                    scale_Array.append(grid_Size - 0.1)  # try minus 0.1 to avoid conductor intersections later
                    x_position += grid_Size
                    conductor_Count += 1
                    x_Counter += 1
                    if x_Counter == number_Of_Cols:
                        print("current column value, ", number_Of_Cols)
                        x_Counter = 0
                        x_position = 0
                        y_position += grid_Size
                        break
                    if conductor_Count == Number_of_conductors:
                        print("conductor count = number of conductors, ", conductor_Count)
                        break
    ...
    for x in range(len(grid_Array)):
        ...
        # additional shapes
        if shape_List[x] == 11:
            print("Square_Fractal conductor created")
            conductor_ = additional_Shapes.create_Square_Fractal_Conductor()
            shape_Array.append("Square_Fractal")

        if shape_List[x] == 12:
            print("Square_Sine conductor created")
            conductor_ = additional_Shapes.create_Square_Sine_Conductor()
            shape_Array.append("Square_Sine")

        if shape_List[x] == 13:
            print("Square_SquareWave conductor created")
            conductor_ = additional_Shapes.create_Square_SquareWave_Conductor()
            shape_Array.append("Square_SquareWave")

        if shape_List[x] == 14:
            print("Square_Saw_1 conductor created")
            conductor_ = additional_Shapes.create_Square_Saw1_Conductor()
            shape_Array.append("Square_Saw_1")

        if shape_List[x] == 15:
            print("Square_Saw_2 conductor created")
            conductor_ = additional_Shapes.create_Square_Saw2_Conductor()
            shape_Array.append("Square_Saw_2")

        if shape_List[x] == 21:
            print("Circle_Fractal conductor created")
            conductor_ = additional_Shapes.create_Circular_Fractal_Conductor()
            shape_Array.append("Circle_Fractal")

        if shape_List[x] == 22:
            print("Circle_Sine conductor created")
            conductor_ = additional_Shapes.create_Circular_Sine_Conductor()
            shape_Array.append("Circle_Sine")

        if shape_List[x] == 23:
            print("Circle_SquareWave conductor created")
            conductor_ = additional_Shapes.create_Circular_SquareWave_Conductor()
            shape_Array.append("Circle_SquareWave")

        if shape_List[x] == 24:
            print("Circle_Saw_1 conductor created")
            conductor_ = additional_Shapes.create_Circular_Saw1_Conductor()
            shape_Array.append("Circle_Saw_1")

        if shape_List[x] == 25:
            print("Circle_Saw_2 conductor created")
            conductor_ = additional_Shapes.create_Circular_Saw2_Conductor()
            shape_Array.append("Circle_Saw_2")

        if shape_List[x] == 31:
            print("Triangle_Fractal conductor created")
            conductor_ = additional_Shapes.create_Triangular_Fractal_Conductor()
            shape_Array.append("Triangle_Fractal")

        if shape_List[x] == 32:
            print("Triangle_Sine conductor created")
            conductor_ = additional_Shapes.create_Triangular_Sine_Conductor()
            shape_Array.append("Triangle_Sine")

        if shape_List[x] == 33:
            print("Triangle_SquareWave conductor created")
            conductor_ = additional_Shapes.create_Triangular_SquareWave_Conductor()
            shape_Array.append("Triangle_SquareWave")

        if shape_List[x] == 34:
            print("Triangle_Saw_1 conductor created")
            conductor_ = additional_Shapes.create_Triangular_Saw1_Conductor()
            shape_Array.append("Triangle_Saw_1")

        if shape_List[x] == 35:
            print("Triangle_Saw_2 conductor created")
            conductor_ = additional_Shapes.create_Triangular_Saw2_Conductor()
            shape_Array.append("Triangle_Saw_2")

        if shape_List[x] == 41:
            print("Hexagon_Fractal conductor created")
            conductor_ = additional_Shapes.create_Hexagon_Fractal_Conductor()
            shape_Array.append("Hexagon_Fractal")

        if shape_List[x] == 42:
            print("Hexagon_Sine conductor created")
            conductor_ = additional_Shapes.create_Hexagon_Sine_Conductor()
            shape_Array.append("Hexagon_Sine")

        if shape_List[x] == 43:
            print("Hexagon_SquareWave conductor created")
            conductor_ = additional_Shapes.create_Hexagon_SquareWave_Conductor()
            shape_Array.append("Hexagon_SquareWave")

        if shape_List[x] == 44:
            print("Hexagon_Saw_1 conductor created")
            conductor_ = additional_Shapes.create_Hexagon_Saw1_Conductor()
            shape_Array.append("Hexagon_Saw_1")

        if shape_List[x] == 45:
            print("Hexagon_Saw_2 conductor created")
            conductor_ = additional_Shapes.create_Hexagon_Saw2_Conductor()
            shape_Array.append("Hexagon_Saw_2")

        if shape_List[x] == 5:
            print("Rhombus conductor created")
            conductor_ = additional_Shapes.create_Rhombus_Conductor()
            shape_Array.append("Rhombus")

        if shape_List[x] == 6:
            print("Pentagon conductor created")
            conductor_ = additional_Shapes.create_Pentagon_Conductor()
            shape_Array.append("Pentagon")

        if shape_List[x] == 7:
            print("Star conductor created")
            conductor_ = additional_Shapes.create_Star_Conductor()
            shape_Array.append("Star")

        if shape_List[x] == 8:
            print("Snowflake conductor created")
            conductor_ = additional_Shapes.create_Snowflake_Conductor()
            shape_Array.append("Snowflake")

        if shape_List[x] == 3:  # is "Triangle" ***
            # had to add in a correction as scaling warps the triangle centroid position 0.0535
            conductor_ = move_Conductor(conductor_, x_Position[x], (y_Position[x] - (scale_List[x] * 0.0535)))
            print("Triangle conductor centroid after moving is : ", x_Position[x], y_Position[x])
        else:
            # shape_Array.append(conductor_Shape) # keep tabs on what each polygon is
            conductor_ = move_Conductor(conductor_, x_Position[x], y_Position[x])

        # shape_Array.append(conductor_Shape) # keep tabs on what each polygon is
        # conductor_ = move_Conductor(conductor_, grid_Array[x].centroid.x, grid_Array[x].centroid.y)
        conductor_Scale = scale_List[x]  # get the scale from optislang for this conductor.
        scale_Array.append(conductor_Scale)  # append the scale to the scale array to be saved later.
        ...
        # rotate the conductor and then add to the rotation array for writing to excel file.

        # rotate the shapes
        if shape_List[x] == 3 or 31 or 32 or 33 or 34 or 35:  # is "Triangle" ***
            # conductor_ = rotate_Special_Conductor(conductor_, rotation_Array[x1], x_Position_Array[x1], (y_Position_Array[x1] - (scale_Array[x1] * 0.0535)))
            conductor_ = rotate_Special_Conductor(conductor_, rotation_List[x], x_Position[x],
                                                  y_Position[x])
            print("Triangle conductor centroid after rotating is : ", conductor_.centroid.x, conductor_.centroid.y)
        else:
    ...
    ax.set_title('Coil Configuration = ' + str(slot_Configuration) + ' Fill Factor = ' + str(
        fill_Factor))  # get this number from the ini file.
    ax.set_xlabel('Slot Width ' + str(Slot_Size_x) + '(mm)')
    ax.set_ylabel('Slot Height ' + str(Slot_Size_Y) + '(mm)')
    set_limits(ax, 0, Slot_Size_x, 0, Slot_Size_Y)
    # set_limits(ax, 0, 150, 0, 150)
    ...
    # conductor_Array_1 # define as global so we can access variable set in script
    ...
    # global conductor_Array_2
    conductor_Array_2 = create_Coil(mirror_Conductor_Array, coil_Center_2, -202.49999999,
                                    conductor_Array_2)  # original is -23, however add -180 to flip slot
    # mirror this slot such that we get shape parity with the turns.

    # save pyplot image
    pyplot.savefig(new_Directory + '/coil_Image_' + str(slot_Configuration),
                   dpi=400)  # this should be a string variable so its easily changed.
    ...
    coil_Script = vbs_Script_Creator_ExperimentVer_2_Extra_Shapes
    coil_Script.create_Dynamic_Vbs_Script(conductor_Array_1, conductor_Array_2, shape_Array, skin_Depth_Array, "true",
                                          vbs_Script_File, new_Directory, slot_Configuration)

    # run vbs script.
    # os.system("") # python will wait until the vb script is finished processing.
    ...
    machine_Losses = get_Losses - 8000  # in watts # could calibrate by taking 3000 watts off, for every 1kw we save roughly 1 %
    ...
    # excel_Tool.write_Coil_To_Excel_Experiment_Version(shape_Array, conductor_Array, scale_Array, rotation_Array, coil_Weight,
                                                      # fill_Factor, machine_Efficiency, machine_Losses, new_Directory,
                                                      # slot_Configuration)

    excel_Tool_Extra_Shapes.write_Coil_To_Excel_Experiment_Version(shape_Array, conductor_Array, scale_Array, rotation_Array, coil_Weight,
                                                      fill_Factor, machine_Efficiency, machine_Losses, new_Directory,
                                                      slot_Configuration)
