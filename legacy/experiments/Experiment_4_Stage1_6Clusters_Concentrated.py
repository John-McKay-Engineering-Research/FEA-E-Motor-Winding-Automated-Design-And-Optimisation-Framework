...
def create_Cluster(number_Of_Conductors, centre_Point_X, centre_Point_Y, cluster_Scale, cluster_Rotation, grid_Array, scale_Array, position_Array,  ax):
    ...
    if number_Of_Conductors < 12 and position_Array == None:
        print("8 grid positions per cluster triggered")
...
def Experiment_4(cluster_Count, cluster_Position_Array, cluster_Scale_Array):
    # create file paths
    ...
    # for 6 clusters need to record cluster positions
    cluster_Positions_Chosen_Array = []
    ...
    for x in range(cluster_Count):
        ...
        cluster_ = create_Cluster(conductors_Per_Cluster, x_Position,
        y_Position, cluster_Scale_Array[x], cluster_rotation, grid_Array, scale_Array, None, ax)

        cluster_Positions_Chosen_Array.append(cluster_)


    for x in range(len(grid_Array)):

        if start_Shape == "Hexagon":
        ...
        conductor_Scale = scale_Array[x]  # get the scale from optislang for this conductor.
        ...
        if start_Shape == "Triangle":  # is "Triangle" ***
            ...
            conductor_ = move_Conductor(conductor_, grid_Array[x].centroid.x,
                                        (grid_Array[x].centroid.y - (scale_Array[x] * 0.19)))  # this might need changed back to 0.0535
    ...
    excel_Tool.write_Coil_To_Excel_Experiment_Version(shape_Array, conductor_Array, scale_Array, None, coil_Weight, fill_Factor, machine_Efficiency, machine_Losses, new_Directory, slot_Configuration)
    ...
    # return the positions chosen for each cluster
    cluster_1_Positions = cluster_Positions_Chosen_Array[0]
    cluster_2_Positions = cluster_Positions_Chosen_Array[1]
    cluster_3_Positions = cluster_Positions_Chosen_Array[2]
    cluster_4_Positions = cluster_Positions_Chosen_Array[3]
    cluster_5_Positions = cluster_Positions_Chosen_Array[4]
    cluster_6_Positions = cluster_Positions_Chosen_Array[5]


    return machine_Efficiency, machine_Losses, cluster_1_Positions, cluster_2_Positions, cluster_3_Positions, cluster_4_Positions, cluster_5_Positions, cluster_6_Positions
